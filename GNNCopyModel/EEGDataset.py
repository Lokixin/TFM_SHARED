...
class EEGGraphDataset(Dataset):
	def __init__(self, X, y, indices, loader_type, sfreq, transform=None):
		...
		self.spec_coh_values = np.load("C:/Users/lokix/OneDrive - Universitat Politècnica de Catalunya/Escritorio/figshare_upload/spec_coh_values.npy", allow_pickle=True)
	...
	def __getitem__(self, idx):
		...
		data = Data(x=node_features, 
					...
					edge_attr=edge_weights,
					dataset_idx=idx, 
					y=torch.tensor([[0, 1]], dtype=torch.float64) if self.labels[idx] == "healthy" else torch.tensor([[1, 0]], dtype=torch.float64)
					# pos=None, norm=None, face=None, **kwargs
					)
		return data